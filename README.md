...
Before deploying this software to real distributed environment, be sure to share the same public and private keys over the PDS's. No Public Key Interface is implemented. Keys automatically generates with ```main.py``` and ```experiment.py```. This encryption is only used for sharing prescriptive statistics.
